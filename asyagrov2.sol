...
contract ASYAGRO is Context, IERC20, Ownable {
  ...
  constructor() public {
    ...
    _symbol = "ASY";
    _decimals = 18;
    _totalSupply = 7500000000 * 10 ** uint256(_decimals);
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(locks[sender].lockedToken > 0){
          uint256 withdrawable = balanceOf(sender) - locks[sender].lockedToken;
  ...
  function transferWithLock(address recipient, uint256 tAmount, uint256 initialLock)  public onlyOwner {
        ...
        locks[recipient] = LockDetails(block.timestamp, initialLock, tAmount, tAmount);
